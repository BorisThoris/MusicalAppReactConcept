...
const useRecordingsPlayer = ({ furthestEndTime, furthestEndTimes }) => {
    const [playbackStatus, setPlaybackStatus] = useState({ currentInstrument: null, isPlaying: false });
    ...
    const { clearAllTimeouts, setNewTimeout } = usePlayback({ playbackStatus: playbackStatus.isPlaying });
    ...
    const playAllOrSpecificInstrumentRecordings = useCallback(
        (instrumentName = null) => {
            ...
            if (shouldPlay) {
                setPlaybackStatus({ currentInstrument: instrumentName, isPlaying: true });
