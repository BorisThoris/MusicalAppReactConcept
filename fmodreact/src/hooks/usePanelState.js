...
const usePanelState = ({ overlapGroups }) => {
  ...
  const {
    index, instrumentName, isOpen, overlapGroup,
  } = state;
  ...
  const openPanel = useCallback(
    ({ index: newIndex, instrumentName: newInstrumentName }) => {
      ...
      const isPanelStateUnchanged = isOpen && index === newIndex && instrumentName === newInstrumentName && overlapGroup === newOverlapGroup;
