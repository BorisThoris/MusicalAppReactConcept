...
const useStageWidth = ({ recordings }) => {
    ...
    useEffect(() => {
        ...
        const endTimesObject = Object.entries(recordings).reduce((acc, [instrumentName, recordingArray]) => {
            const maxEndTime = recordingArray.reduce((max, recording) => Math.max(max, recording.endTime), 0);
