        ...
        Object.keys(updatedRecordings).forEach((instrumentName) => {
          ...
          if (recordingIndex !== -1) {
            const updatedRecording = {
              ...
              params: targetList[recordingIndex].params.map((param) => (param.name === updatedParam.name ? updatedParam : param)),
            };
        ...
        Object.keys(updatedRecordings).forEach((instrumentName) => {
          ...
          if (parent && parent.events && parent.events.length > 1) {
            ...
            if (updatedEvents.length !== parent.events.length) {
              ...
              updatedRecordings[instrumentName] = recordings.map((group) => (group === parent ? { ...group, events: updatedEvents } : group));
            }
          } else {
