...
var BANK_COUNT = 4
var BANK_NAME = ['SFX.bank', 'Music.bank', 'Vehicles.bank', 'VO.bank']
...
function prerun() {
  var fileUrl = '/assets/html5/media/'
  var fileName
  var folderName = '/'
  var canRead = true
  var canWrite = false
...
function customFileRead(handle, buffer, sizebytes, bytesread, userdata) {
  ...
  var result = FMOD.file_read(
    handle,
    buffer_outval,
    sizebytes,
    bytesread_outval,
  ) // read produces a new array with data.
  if (result == FMOD.OK) {
...
function loadBank(method) {
  ...
  if (method == 0.0) {
  ...
  } else if (method == 1.0 || method == 2.0) {
    // 1 == LoadBank_MemoryPoint, 2 == LoadBank_MemoryPoint
    var memoryPtr
    var memoryLength
    var memoryMode

    result = FMOD.ReadFile(gSystemCore, '/' + filename, outval) // Use a little helper function to load a file into memory.
    CHECK_RESULT(result)
    ...
    memoryMode =
      method == 2.0 ? FMOD.STUDIO_LOAD_MEMORY_POINT : FMOD.STUDIO_LOAD_MEMORY

    result = gSystem.loadBankMemory(
      memoryPtr,
      memoryLength,
      memoryMode,
      FMOD.STUDIO_LOAD_BANK_NONBLOCKING,
...
function updateApplication() {
  ...
  for (i = 0; i < BANK_COUNT; ++i) {
    ...
    if (bankLoadState[i] == FMOD.STUDIO_LOADING_STATE_LOADED) {
      ...
      if (
        gWantSampleLoad &&
        sampleLoadState[i] == FMOD.STUDIO_LOADING_STATE_UNLOADED
      ) {
        CHECK_RESULT(gBank[i].loadSampleData())
      } else if (
        !gWantSampleLoad &&
        (sampleLoadState[i] == FMOD.STUDIO_LOADING_STATE_LOADING ||
          sampleLoadState[i] == FMOD.STUDIO_LOADING_STATE_LOADED)
      ) {
  ...
  for (i = 0; i < BANK_COUNT; ++i) {
    ...
    ;(document.getElementById(
      'bank' + i + '_handle',
    ).innerHTML = getHandleStateString(gBank[i])),
      (document.getElementById(
        'bank' + i + '_bankstate',
      ).innerHTML = getLoadingStateString(
        bankLoadState[i],
        loadStateResult[i],
      )),
      (document.getElementById(
  ...
  document.getElementById('sampletogglestate').innerHTML = gWantSampleLoad
    ? ' ON'
    : ' OFF'
