...
const FMOD = {}; // FMOD global object which must be declared to enable 'main' and 'preRun' and then call the constructor function.
...
let gSystem; // Global 'System' object which has the top level API functions.  Sounds and channels are created from this.
const gSound = {}; // Array of 5 sounds.
const gChannel = {}; // Array of handles to channels that are playing
const gGroup = {}; // Array of All channelgroups, 0 and 1 as the channelgroups that live under the master as children, and 2 as the master group.
...
function prerun() {
  ...
  for (let count = 0; count < fileName.length; count++) {
...
function volumeChanged(groupid, val) {
  ...
  if (gGroup[groupid]) {
    const result = gGroup[groupid].setVolume(parseFloat(val));
    CHECK_RESULT(result);
  }
}
